...
broadcast_device_info broadcasters[100];
int broadcaster_count = 0;

// Extended scanning commands and structure
typedef struct {
...
int set_extended_scan_parameters(int device) {
    int status;
...
int set_event_mask(int device) {
    int status;
    uint8_t le_event_mask[8] = {0xFF, 0xFF, 0xFF, 0xFF, 0x1F, 0x1F, 0x1F, 0x1F};
...
int set_extended_enable_scan(int device, uint8_t enable) {
    int status;
...
int device_found = 0;
static void parse_data(uint8_t *eir, size_t eir_len,
...
void scan_for_ext_advertisements(int device) {
    ...
    while (last_detection_time - now < 5 && count < 100) {
        ...
        if (meta_event->subevent == EVT_EXT_LE_ADVERTISING_REPORT) {
            ...
            while (reports_count--) {
                ...
                parse_data(info->data, info->data_len, name, sizeof(name));
...
int main() {
    ...
    if (set_extended_scan_parameters(device) < 0) {
        hci_close_dev(device);
        return 0;
    }
    ...
    if (set_extended_enable_scan(device, 0x01) < 0) {
        hci_close_dev(device);
        return 0;
    }

    set_extended_enable_scan(device, 0x00);
    scan_for_ext_advertisements(device);
    ...
    set_extended_enable_scan(device, 0x01);
